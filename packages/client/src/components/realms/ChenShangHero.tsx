...
const RIVER_COL = ['w', 'y', 'b', 'v', 't', 'm', 'w', 'b', 'v', 't', 'y', 'm'];
const AMB_COL = ['w', 'w', 'y', 'b', 'v', 't', 'w', 'y'];
const FIELD_MIX = ['w', 'w', 'w', 'y', 'b', 'b', 'v', 't', 'w', 'y', 'w', 'i'];
const FLOW_MIX = ['w', 'w', 'y', 'b', 'b', 'v', 't', 'w', 'y', 'i'];
const SPARK_MIX = ['w', 'y', 'b', 'v', 't', 'w'];
const NEB_KEYS = ['cb', 'uq', 'pq', 'tq', 'vp', 'cb', 'uq', 'tq', 'vp', 'cb'];
const SPH_POOL = ['w', 'y', 'b', 'v', 'w', 'y', 'b', 't'];
const BG_MIX = ['cb', 'cb', 'uq', 'vp', 'vp', 'pq', 'tq', 'mg', 'cb', 'vp', 'tq', 'mg', 'yl'];

// ── 工具函数 ──
const rnd = (a: number, b: number) => a + Math.random() * (b - a);
const gauss = () => (Math.random() + Math.random() + Math.random() - 1.5) / 1.5;
const easeIO = (p: number) => (p < 0.5 ? 4 * p * p * p : 1 - Math.pow(-2 * p + 2, 3) / 2);
const easeO = (p: number) => 1 - Math.pow(1 - p, 3);
const clamp = (v: number, a: number, b: number) => (v < a ? a : v > b ? b : v);

function jit(c: string, amt: number) {
  return c.split(',').map(Number).map((v) => clamp(Math.round(v + (Math.random() - 0.5) * 2 * amt), 0, 255)).join(',');
}
function darkOf(c: string) {
...
function makeGlow(c: string): Sprite[] {
  const out: Sprite[] = [];
  for (let v = 0; v < 3; v++) {
    const col = jit(c, 16);
...
function makeCloud(c: string): Sprite[] {
  const out: Sprite[] = [];
  for (let v = 0; v < 2; v++) {
    const col = jit(c, 12);
    const s = document.createElement('canvas');
...
function makeSphere(c: string, dk: string): Sprite {
  const s = document.createElement('canvas');
  s.width = 64; s.height = 64;
  const g = s.getContext('2d')!;
...
function makeStroke(c: string): Sprite {
  const col = jit(c, 10);
  const s = document.createElement('canvas');
...
function makeDab(c: string): Sprite[] {
  const out: Sprite[] = [];
  for (let v = 0; v < 3; v++) {
    const col = jit(c, 14);
    const s = document.createElement('canvas');
...
// ── 绘制工具 ──
function stampInk(g: CanvasRenderingContext2D, x: number, y: number, r: number, rgb: string, alpha: number) {
  if (r < 0.6) return;
...
function dashStroke(g: CanvasRenderingContext2D, x: number, y: number, ang: number, len: number, w: number, rgb: string, alpha: number) {
...
function paintVortex(g: CanvasRenderingContext2D, cx: number, cy: number, R: number, rgb: string, rgb2: string, turns: number, dir: number) {
  const steps = (R * 0.9) | 0;
...
function paintSplash(g: CanvasRenderingContext2D, x: number, y: number, sc: number, rgb: string, rgb2: string, a: number, dir: number) {
