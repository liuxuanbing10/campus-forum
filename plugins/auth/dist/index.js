import { uid, signJwt } from '@campus-forum/core';
...
function getDeviceCode(request) {
...
export const authPlugin = {
    ...
    apply(ctx) {
        ...
        app.post('/api/auth/register', async (request, reply) => {
            ...
            // 4. 检查设备码是否已被绑定
            const boundDevice = db.get('SELECT id, username FROM users WHERE device_code = ?', deviceCode);
            if (boundDevice) {
                return reply.status(409).send({ error: '该设备码已被绑定到其他账号，一个设备码只能登录一个账号' });
            }
            // 5. 创建用户
            const hash = await bcrypt.hash(password, 10);
            db.run('INSERT INTO users (username, password_hash, display_name, email, device_code) VALUES (?, ?, ?, ?, ?)', username, hash, username, email || null, deviceCode);
        ...
        app.post('/api/auth/login', async (request, reply) => {
            ...
            // 1. 查找用户
            const user = db.get('SELECT id, username, password_hash, display_name FROM users WHERE username = ?', username);
            if (!user) {
        ...
        app.get('/api/auth/me', async (request, reply) => {
            let userId = request.session?.userId || uid(request);
            if (!userId) {
                return reply.status(401).send({ error: '未登录' });
            }
            // 更新在线时间
            db.run("UPDATE users SET last_active_at = datetime('now') WHERE id = ?", userId);
            const user = db.get('SELECT id, username, display_name, is_admin FROM users WHERE id = ?', userId);
            if (!user) {
        ...
        app.put('/api/auth/me', async (request, reply) => {
            const userId = request.session?.userId || uid(request);
            if (!userId) {
                return reply.status(401).send({ error: '未登录' });
            }
            const { display_name, email, avatar_url } = request.body;
            const user = db.get('SELECT id, username FROM users WHERE id = ?', request.session.userId);
            if (!user) {
                return reply.status(401).send({ error: '用户不存在' });
            }
            const updates = [];
            ...
            const updatedUser = db.get('SELECT id, username, display_name, email, avatar_url, is_admin, role, is_banned, created_at FROM users WHERE id = ?', request.session.userId);
            return {
                success: true,
                message: '资料更新成功',
                user: {
        ...
        app.put('/api/auth/password', async (request, reply) => {
            ...
            const user = db.get('SELECT id, password_hash FROM users WHERE id = ?', request.session.userId);
            if (!user) {
                return reply.status(401).send({ error: '用户不存在' });
            }
        ...
        app.get('/api/users/:id', async (req, rep) => {
            ...
            const user = db.get('SELECT id,username,display_name,bio,created_at,last_active_at,points FROM users WHERE id=?', id);
            if (!user)
                return rep.status(404).send({ error: '用户不存在' });
            const postCount = db.get('SELECT COUNT(*) as c FROM posts WHERE author_id=?', id).c;
            const commentCount = db.get('SELECT COUNT(*) as c FROM comments WHERE author_id=?', id).c;
            const followerCount = db.get('SELECT COUNT(*) as c FROM follows WHERE followed_id=?', id).c;
            const followingCount = db.get('SELECT COUNT(*) as c FROM follows WHERE user_id=?', id).c;
            const recentPosts = db.all('SELECT id,title,created_at,board_id FROM posts WHERE author_id=? ORDER BY created_at DESC LIMIT 10', id);
        ...
        app.post('/api/auth/oauth/bind', async (req, rep) => {
            ...
            if (!userId)
                return rep.status(401).send({ error: '请先登录' });
            const { provider, providerId } = req.body;
            if (!provider || !providerId)
                return rep.status(400).send({ error: '参数不完整' });
            try {
                db.run('INSERT INTO oauth_accounts (user_id,provider,provider_id) VALUES (?,?,?)', userId, provider, providerId);
            }
            catch {
                return rep.status(409).send({ error: '已绑定' });
            }
        ...
        function httpsPost(url, body, form = false) {
            return new Promise((resolve, reject) => {
                const u = new URL(url);
        ...
        const OAUTH_PROVIDERS = {
            github: {
                ...
                exchangeToken: async (id, secret, code, uri) => {
                    const res = await httpsPost('https://github.com/login/oauth/access_token', JSON.stringify({ client_id: id, client_secret: secret, code, redirect_uri: uri }));
            ...
            qq: {
                ...
                getUserInfo: async (token, clientId) => {
                    ...
                    if (!openid)
                        throw new Error('get_openid_failed');
        ...
        // 为每个提供商注册路由
        for (const provider of ['github', 'qq', 'weixin']) {
            ...
            app.get(`/api/auth/oauth/${provider}/url`, async (req, rep) => {
                ...
                if (!clientId)
                    return rep.status(500).send({ error: `${envId} 未配置` });
            ...
            app.get(`/api/auth/oauth/${provider}/callback`, async (req, rep) => {
                ...
                if (oauthError)
                    return rep.redirect(`/?oauth_error=${oauthError}`);
                if (!code)
                    return rep.status(400).send({ error: '缺少授权码' });
                const clientId = process.env[envId];
                ...
                try {
                    ...
                    const existing = db.get('SELECT user_id FROM oauth_accounts WHERE provider=? AND provider_id=?', provider, userInfo.id);
                    // 绑定模式
                    if (state === 'bind') {
                        const loggedUserId = uid(req);
                        if (existing) {
                            if (existing.user_id === loggedUserId)
                                return rep.redirect(`${frontendUrl}/settings?tab=oauth&msg=already_bound`);
                            return rep.redirect(`${frontendUrl}/settings?tab=oauth&msg=bound_by_other`);
                        }
                        if (!loggedUserId)
                            return rep.redirect(`${frontendUrl}/login`);
                    ...
                    // 登录模式：已有绑定
                    if (existing) {
                        const user = db.get('SELECT id, username FROM users WHERE id=?', existing.user_id);
                        if (user) {
        ...
        app.post('/api/auth/oauth/complete', async (req, rep) => {
            ...
            if (!token || !username)
                return rep.status(400).send({ error: '参数不完整' });
            if (username.length < 2 || username.length > 20)
                return rep.status(400).send({ error: '用户名长度应为 2-20 个字符' });
            const store = oauthTempStore.get(token);
            if (!store)
                return rep.status(400).send({ error: 'token 无效或已过期' });
            ...
            const existingUser = db.get('SELECT id FROM users WHERE username=?', username);
            if (existingUser)
                return rep.status(409).send({ error: '用户名已存在' });
            ...
            const user = db.get('SELECT id, username FROM users WHERE username=?', username);
            if (!user)
                return rep.status(500).send({ error: '创建用户失败' });
            try {
                db.run('INSERT INTO oauth_accounts (user_id, provider, provider_id) VALUES (?, ?, ?)', user.id, store.provider, store.providerUserId);
            }
            catch { /* ok */ }
            req.session.userId = user.id;
            req.session.username = user.username;
        ...
        app.post('/api/users/avatar', async (req, rep) => {
            ...
            if (!image)
                return rep.status(400).send({ error: '请提供图片数据' });
            const m = image.match(/^data:(image\/\w+);base64,(.+)$/);
            if (!m)
                return rep.status(400).send({ error: '图片格式错误' });
            const ext = m[1].split('/')[1].replace('jpeg', 'jpg');
            const buf = Buffer.from(m[2], 'base64');
            if (buf.length > 2 * 1024 * 1024)
                return rep.status(400).send({ error: '图片不能超过 2MB' });
        ...
        app.post('/api/auth/send-verify-email', async (req, rep) => {
            ...
            if (!email || !email.includes('@'))
                return rep.status(400).send({ error: '邮箱格式不正确' });
        ...
        app.post('/api/auth/verify-captcha', async (req, rep) => {
            ...
            if (!captcha)
                return rep.status(400).send({ error: '请输入验证码' });
            // 演示：任何 4 位字符都通过，实际应集成 reCAPTCHA 等
            if (captcha.length < 4)
                return rep.status(400).send({ error: '验证码错误' });
            return { success: true };
        });
    },
};
