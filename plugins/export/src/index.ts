...
const posts=db.all('SELECT id,title,content,board_id,created_at FROM posts WHERE author_id=? ORDER BY created_at DESC',userId);
const comments=db.all('SELECT c.id,c.content,c.post_id,p.title as post_title,c.created_at FROM comments c JOIN posts p ON c.post_id=p.id WHERE c.author_id=? ORDER BY c.created_at DESC',userId);
