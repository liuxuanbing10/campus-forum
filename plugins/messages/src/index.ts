...
if(!(await db.get('SELECT id FROM users WHERE id=?',receiverId)))return rep.status(404).send({error:'用户不存在'});
...
if(!conv){await db.run('INSERT INTO conversations (user1_id,user2_id) VALUES (?,?)',u1,u2);conv=(await db.get<{id:number}>('SELECT id FROM conversations ORDER BY id DESC LIMIT 1'))!;}
...
// WebSocket 实时推送
const sender = await db.get<{username:string,display_name:string|null}>('SELECT username,display_name FROM users WHERE id=?',userId);
...
const userId=uid(req);if(!userId)return rep.status(401).send({error:'请先登录'});
return {conversations:await db.all<any>(`SELECT c.id,c.last_message,c.last_message_at,
...
const conv=await db.get<{user1_id:number;user2_id:number}>('SELECT * FROM conversations WHERE id=?',convId);
...
return {messages:await db.all<any>('SELECT m.*,u.username as sender_name FROM messages m JOIN users u ON m.sender_id=u.id WHERE m.conversation_id=? ORDER BY m.created_at ASC',convId)};
});

app.get('/api/messages/unread-count',async(req,rep)=>{
const userId=uid(req);if(!userId)return rep.status(401).send({error:'请先登录'});
const r=await db.get<{c:number}>('SELECT COUNT(*) as c FROM messages m JOIN conversations c ON m.conversation_id=c.id WHERE (c.user1_id=? OR c.user2_id=?) AND m.sender_id!=? AND m.is_read=0',userId,userId,userId);
