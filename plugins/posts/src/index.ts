...
export const postsPlugin: Plugin = {
  ...
  apply(ctx: PluginContext) {
    ...
    app.post('/api/boards', async (req, rep) => {
      ...
      await db.run('INSERT INTO boards (name, description, icon, created_by) VALUES (?,?,?,?)', name, description || '', icon || '📁', userId);
      return { success: true, board: await db.get<BoardRow>('SELECT * FROM boards ORDER BY id DESC LIMIT 1') };
    });
    ...
    app.put('/api/boards/:id', async (req, rep) => {
      ...
      if (!(await db.get('SELECT id FROM boards WHERE id = ?', id))) return rep.status(404).send({ error: '版块不存在' });
      const body = req.body as any;
      if (body.name) await db.run('UPDATE boards SET name = ? WHERE id = ?', body.name, id);
      if (body.description !== undefined) await db.run('UPDATE boards SET description = ? WHERE id = ?', body.description, id);
      if (body.icon !== undefined) await db.run('UPDATE boards SET icon = ? WHERE id = ?', body.icon, id);
    ...
    app.post('/api/posts', async (req, rep) => {
      ...
      if (!(await db.get('SELECT id FROM boards WHERE id = ?', boardId))) return rep.status(404).send({ error: '版块不存在' });
      // 敏感词检查
      ...
      await db.run('INSERT INTO posts (title, content, author_id, board_id, is_anonymous, is_private, images, is_pending) VALUES (?,?,?,?,?,?,?,?)',
    ...
    app.put('/api/posts/:id', async (req, rep) => {
      const userId = uid(req); if (!userId) return rep.status(401).send({ error: '请先登录' });
      const id = Number((req.params as { id: string }).id);
      const post = await db.get<PostRow>('SELECT * FROM posts WHERE id = ?', id);
      ...
      // 保存编辑历史
      const orig = await db.get<{ title: string; content: string }>('SELECT title,content FROM posts WHERE id=?', id);
      if (orig) await db.run('INSERT INTO post_versions (post_id,title,content,edited_by) VALUES (?,?,?,?)', id, orig.title, orig.content, userId);
      if (data.title !== undefined) await db.run('UPDATE posts SET title = ? WHERE id = ?', data.title, id);
      if (data.content !== undefined) await db.run('UPDATE posts SET content = ? WHERE id = ?', data.content, id);
      if (data.boardId !== undefined) await db.run('UPDATE posts SET board_id = ? WHERE id = ?', data.boardId, id);
      if (data.isAnonymous !== undefined) await db.run('UPDATE posts SET is_anonymous = ? WHERE id = ?', data.isAnonymous ? 1 : 0, id);
    ...
    app.put('/api/posts/:id/pin', async (req, rep) => {
      ...
      if (!(await isAdmin(db, userId))) return rep.status(403).send({ error: '仅管理员可操作' });
      const id = Number((req.params as { id: string }).id);
      const post = await db.get<{ id: number; is_pinned: number }>('SELECT id, is_pinned FROM posts WHERE id = ?', id);
      if (!post) return rep.status(404).send({ error: '帖子不存在' });
      const newVal = post.is_pinned ? 0 : 1;
      await db.run('UPDATE posts SET is_pinned = ? WHERE id = ?', newVal, id);
    ...
    app.put('/api/posts/:id/privacy', async (req, rep) => {
      const userId = uid(req); if (!userId) return rep.status(401).send({ error: '请先登录' });
      const id = Number((req.params as { id: string }).id);
      const post = await db.get<{ id: number; author_id: number; is_private: number }>('SELECT id, author_id, is_private FROM posts WHERE id = ?', id);
      if (!post) return rep.status(404).send({ error: '帖子不存在' });
      if (post.author_id !== userId) return rep.status(403).send({ error: '仅作者可操作' });
      const newVal = post.is_private ? 0 : 1;
      await db.run('UPDATE posts SET is_private = ?, updated_at = datetime(\'now\') WHERE id = ?', newVal, id);
    ...
    app.delete('/api/posts/:id', async (req, rep) => {
      ...
      await db.run('DELETE FROM votes WHERE post_id = ?', id);
    ...
    app.post('/api/posts/:id/comments', async (req, rep) => {
      ...
      if (!(await db.get('SELECT id FROM posts WHERE id = ?', postId))) return rep.status(404).send({ error: '帖子不存在' });
      if (parentId && !(await db.get('SELECT id FROM comments WHERE id = ? AND post_id = ?', parentId, postId)))
      ...
      await db.run("INSERT INTO comments (content, author_id, post_id, parent_id, is_anonymous) VALUES (?,?,?,?,?)",
        content.trim(), userId, postId, parentId || null, isAnonymous ? 1 : 0);
      // 积分：评论+2分
      await addPoints(db, userId, 2);
      // 更新帖子最新回复时间
      await db.run("UPDATE posts SET last_replied_at = datetime('now') WHERE id = ?", postId);
      const comment = await db.get<CommentRow>('SELECT * FROM comments ORDER BY id DESC LIMIT 1');

      // 🔔 通知帖子作者
      const postAuthor = await db.get<{ author_id: number }>('SELECT author_id FROM posts WHERE id = ?', postId);
      ...
      // 🔔 通知被回复的评论作者
      if (parentId) {
        const parentAuthor = await db.get<{ author_id: number }>('SELECT author_id FROM comments WHERE id = ?', parentId);
      ...
      for (const name of mentions) {
        const u = await db.get<{ id: number }>('SELECT id FROM users WHERE username=?', name);
    ...
    app.delete('/api/comments/:id', async (req, rep) => {
      const userId = uid(req); if (!userId) return rep.status(401).send({ error: '请先登录' });
      const id = Number((req.params as { id: string }).id);
      const c = await db.get<CommentRow>('SELECT id, author_id FROM comments WHERE id = ?', id);
    ...
    app.put('/api/comments/:id', async (req, rep) => {
      ...
      await db.run("UPDATE comments SET content=?, edited_at=datetime('now') WHERE id=?", content.trim(), id);
    ...
    app.post('/api/votes', async (req, rep) => {
      ...
      if (!(await db.get(`SELECT id FROM ${target.table} WHERE id = ?`, target.id)))
        return rep.status(404).send({ error: `${target.table === 'posts' ? '帖子' : '评论'}不存在` });
      if (value === 0) { await db.run(`DELETE FROM votes WHERE user_id=? AND ${target.col}=?`, userId, target.id); return { success: true, message: '已取消' }; }
      const existing = await db.get<{ id: number; value: number }>(`SELECT id,value FROM votes WHERE user_id=? AND ${target.col}=?`, userId, target.id);
      if (existing) {
        if (existing.value === value) { await db.run('DELETE FROM votes WHERE id=?', existing.id); return { success: true, message: `已取消${value===1?'点赞':'踩'}` }; }
        await db.run('UPDATE votes SET value=? WHERE id=?', value, existing.id);
      } else { await db.run(`INSERT INTO votes (user_id,${target.col},value) VALUES (?,?,?)`, userId, target.id, value); }
    ...
    app.post('/api/favorites', async (req, rep) => {
      ...
      if (!(await db.get('SELECT id FROM posts WHERE id = ?', postId))) return rep.status(404).send({ error: '帖子不存在' });
      const existing = await db.get<{ id: number }>('SELECT id FROM favorites WHERE user_id=? AND post_id=?', userId, postId);
      if (existing) { await db.run('DELETE FROM favorites WHERE id=?', existing.id); return { success: true, isFavorited: false, message: '已取消收藏' }; }
    ...
    app.get('/api/posts/:id/stats', async (req) => {
      const id = Number((req.params as { id: string }).id);
      return (await db.get<{ like_count: number; comment_count: number; favorite_count: number; view_count: number }>(
    ...
    app.get('/api/posts/:id/tags', async (req) => {
      const id = Number((req.params as { id: string }).id);
      return { tags: await db.all<{ id: number; name: string }>('SELECT t.id,t.name FROM tags t JOIN post_tags pt ON t.id=pt.tag_id WHERE pt.post_id=?', id) };
    });

    // ─── 编辑历史 ───
    app.get('/api/posts/:id/versions', async (req, rep) => {
      const id = Number((req.params as { id: string }).id);
      if (!(await db.get('SELECT id FROM posts WHERE id=?', id))) return rep.status(404).send({ error: '帖子不存在' });
      return { versions: await db.all<any>('SELECT v.id,v.title,v.content,v.created_at,u.username as editor_name FROM post_versions v JOIN users u ON v.edited_by=u.id WHERE v.post_id=? ORDER BY v.created_at DESC', id) };
    });
    ...
    app.post('/api/posts/:id/tags', async (req, rep) => {
      ...
      const tag = await db.get<{ id: number }>('SELECT id FROM tags WHERE name=?', name.trim());
      let tagId: number;
      if (tag) { tagId = tag.id; } else { await db.run('INSERT INTO tags (name) VALUES (?)', name.trim()); tagId = (await db.get<{ id: number }>('SELECT id FROM tags ORDER BY id DESC LIMIT 1'))!.id; }
      try { await db.run('INSERT INTO post_tags (post_id,tag_id) VALUES (?,?)', postId, tagId); } catch { return rep.status(409).send({ error: '标签已存在' }); }
    ...
    app.delete('/api/posts/:id/tags/:tagId', async (req, rep) => {
      ...
      await db.run('DELETE FROM post_tags WHERE post_id=? AND tag_id=?', postId, tagId);
      return { success: true };
    });
    ...
    app.put('/api/admin/posts/:id/review', async (req, rep) => {
      ...
      if (action === 'reject') { await db.run('DELETE FROM posts WHERE id=?', id); return { success: true, message: '已拒绝' }; }
      await db.run('UPDATE posts SET is_pending=0 WHERE id=?', id);
    ...
    app.get('/api/admin/sensitive-words', async (req, rep) => {
      const u = uid(req); if (!u || !(await isAdmin(db, u))) return rep.status(403).send({ error: '仅管理员可查看' });
      return { words: await db.all<any>('SELECT id,word,created_at FROM sensitive_words ORDER BY created_at DESC') };
    });
    ...
    app.post('/api/admin/sensitive-words', async (req, rep) => {
      ...
      try { await db.run('INSERT INTO sensitive_words (word) VALUES (?)', word.trim()); } catch { return rep.status(409).send({ error: '敏感词已存在' }); }
      return { success: true };
    });

    app.delete('/api/admin/sensitive-words/:id', async (req, rep) => {
      const u = uid(req); if (!u || !(await isAdmin(db, u))) return rep.status(403).send({ error: '仅管理员可操作' });
      await db.run('DELETE FROM sensitive_words WHERE id=?', Number((req.params as { id: string }).id));
      return { success: true };
    });
  },
};
