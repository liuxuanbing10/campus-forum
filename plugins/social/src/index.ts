...
export const socialPlugin: Plugin = {
  ...
  apply(_ctx: PluginContext) {
    ...
    app.post('/api/follow', async (req, rep) => {
      ...
      const target = await db.get<{ id: number }>('SELECT id FROM users WHERE id=?', followedId);
      if (!target) return rep.status(404).send({ error: '用户不存在' });
      const existing = await db.get<{ id: number }>('SELECT id FROM follows WHERE user_id=? AND followed_id=?', userId, followedId);
      if (existing) return rep.status(409).send({ error: '已关注' });
      await db.run('INSERT INTO follows (user_id,followed_id) VALUES (?,?)', userId, followedId);
    ...
    app.delete('/api/follow', async (req, rep) => {
      ...
      const { followedId } = req.body as { followedId: number };
      if (!followedId) return rep.status(400).send({ error: '缺少 followedId' });
      await db.run('DELETE FROM follows WHERE user_id=? AND followed_id=?', userId, followedId);
    ...
    app.get('/api/users/:id/followers', async (req, rep) => {
      ...
      return { followers: await db.all<any>('SELECT f.id,f.created_at,u.id as user_id,u.username FROM follows f JOIN users u ON f.user_id=u.id WHERE f.followed_id=?', id) };
    });

    app.get('/api/users/:id/following', async (req, rep) => {
      const id = Number((req.params as { id: string }).id);
      if (!(await db.get('SELECT id FROM users WHERE id=?', id))) return rep.status(404).send({ error: '用户不存在' });
      return { following: await db.all<any>('SELECT f.id,f.created_at,u.id as user_id,u.username FROM follows f JOIN users u ON f.followed_id=u.id WHERE f.user_id=?', id) };
    });
    ...
    app.get('/api/follow/check', async (req, rep) => {
      ...
      const f = await db.get('SELECT id FROM follows WHERE user_id=? AND followed_id=?', userId, targetId);
    ...
    app.post('/api/reports', async (req, rep) => {
      ...
      const existing = await db.get('SELECT id FROM reports WHERE reporter_id=? AND target_type=? AND target_id=? AND status=\'pending\'', userId, targetType, targetId);
      if (existing) return rep.status(409).send({ error: '已举报过' });
      await db.run('INSERT INTO reports (reporter_id,target_type,target_id,reason) VALUES (?,?,?,?)', userId, targetType, targetId, reason.trim());
    ...
    app.get('/api/admin/reports', async (req, rep) => {
      ...
      return { reports: await db.all<any>('SELECT r.*,ru.username as reporter_name FROM reports r JOIN users ru ON r.reporter_id=ru.id ORDER BY r.created_at DESC LIMIT 20 OFFSET ?', (page-1)*20), page };
    });
    ...
    app.get('/api/users/:id/points', async (req, rep) => {
      const id = Number((req.params as { id: string }).id);
      const u = await db.get<{ points: number; created_at: string }>('SELECT COALESCE(points,0) as points,created_at FROM users WHERE id=?', id);
    ...
    app.get('/api/users/:id/posts', async (req, rep) => {
      ...
      if (!(await db.get('SELECT id FROM users WHERE id=?', id))) return rep.status(404).send({ error: '用户不存在' });
      const page = Math.min(100, Math.max(1, Number((req.query as any).page) || 1));
      const posts = await db.all<any>(`SELECT p.id,p.title,p.created_at,b.name as board_name,COALESCE(l.like_count,0) as like_count,COALESCE(c.comment_count,0) as comment_count
    ...
    app.get('/api/users/:id/comments', async (req, rep) => {
      ...
      if (!(await db.get('SELECT id FROM users WHERE id=?', id))) return rep.status(404).send({ error: '用户不存在' });
      const page = Math.min(100, Math.max(1, Number((req.query as any).page) || 1));
      const comments = await db.all<any>(`SELECT c.id,c.content,c.created_at,p.id as post_id,p.title as post_title
    ...
    app.get('/api/admin/audit-logs', async (req, rep) => {
      const u = uid(req); if (!u || !(await isAdmin(db, u))) return rep.status(403).send({ error: '仅管理员可查看' });
      const page = Math.min(100, Math.max(1, Number((req.query as any).page) || 1));
      return { logs: await db.all<any>('SELECT l.*,a.username as admin_name FROM audit_logs l JOIN users a ON l.admin_id=a.id ORDER BY l.created_at DESC LIMIT 30 OFFSET ?', (page-1)*30), page };
    });
  },
};
